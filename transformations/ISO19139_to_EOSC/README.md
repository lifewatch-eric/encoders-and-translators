...
| Version | 1.0.0 |
| Status | 🚧 In progress — mapping complete, core fields implemented; several optional EOSC fields are intentionally unmapped (see [Known Limitations](docs/mapping-notes.md#known-limitations)) |
| Program | EOSC Beyond |
...
| `publicContacts` | every `electronicMailAddress` in the record |
| `webpage`, `urls[0]` | `$catalogue-base-url` + `fileIdentifier` |
| `urls[1..]` | `distributionInfo//CI_OnlineResource/linkage/URL` |
...
| `type`, `nodePID`, `resourceOwner`, `logo`, `scientificDomains`, `accessTypes`, `jurisdiction` | fixed defaults (parameterised) |
...
|---|---|---|
| `$catalogue-base-url` | `https://metadatacatalogue.lifewatch.eu/srv/api/records/` | Prefixed to `fileIdentifier` to build `webpage` / `urls[0]` |
...
| `$access-type` | `access_type-virtual` | Value written to `accessTypes` |
...
mapping notes — notably: `id` is never emitted (EOSC assigns it on registration),
`categories` depends on a manual `$service-category` parameter since ISO 19139 has no
equivalent field, and `trl` requires the non-standard `serviceTRL_service` extension
element to be present in the source record.

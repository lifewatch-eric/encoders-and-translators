#!/usr/bin/env python3
"""
transformations/ISO19139_to_DataCite401/run.py
=============================================
One-command runner for the ISO 19139 → DataCite 4.0.1 (OpenAIRE) transformation.
...
─────
    ...
    python3 run.py --resource-type VRE               # default: Workflow
...
def input_to_output_name(name: str) -> str:
    ...
    for old in ("iso19139", "iso-19139", "iso_19139", "ISO19139"):
        if old in stem:
            stem = stem.replace(old, "datacite401", 1)
            break
    else:
        stem = stem + "_datacite401"
    return stem + ".xml"
...
# ── Test runner ────────────────────────────────────────────────────────────────
def run_tests(input_path, resource_type, quiet):
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Auto-transform ISO 19139 → DataCite 4.0.1 and run tests.",
    ...
    p.add_argument("--resource-type", default="Workflow", metavar="TYPE",
        choices=["Workflow", "VRE"],
        help="Selects the fixed resourceType pair (default: Workflow)")
    p.add_argument("--publisher", default=None, metavar="NAME",
        help="Override default publisher (default: 'LifeWatch ERIC')")
    p.add_argument("--catalogue-base-url", default=None, metavar="URL",
        help="Override the base URL used for the DOI-less identifier fallback")
...
def main():
    ...
    banner("ISO 19139 → DataCite 4.0.1  ·  Transform & Test Runner")
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    print(f"  {DIM}Started    : {ts}{RESET}")
    print(f"  {DIM}Stylesheet : {XSL_PATH}{RESET}")
    print(f"  {DIM}resource-type : {args.resource_type}{RESET}")
    ...
    xslt_params = {"resource-type": etree.XSLT.strparam(args.resource_type)}
    if args.publisher:
    ...
    if not args.no_tests:
        ...
        if inputs_to_test:
            ...
            for inp in inputs_to_test:
                ...
                p, f = run_tests(inp, args.resource_type, args.quiet)
